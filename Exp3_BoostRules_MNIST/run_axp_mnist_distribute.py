...
if __name__ == '__main__':
    ...
    if (encoding is None) and (wd is None) and (pb_encoding is None):
        # mfile = './mnist_boomers_251119/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2.mod.pkl'.format(base, num, depth)
        ## 260504: for REs with large max_conds
        mfile = './mnist_boomers_260505_lcm/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2.mod.pkl'.format(base, num, depth)
    else:
        ## 251209: thinking about different wd
        # mfile = './mnist_boomers_251119/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2_wd_{3}.mod.pkl'.format(base, num, depth, wd)
        ## 260504: for REs with large max_conds
        mfile = './mnist_boomers_260505_lcm/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2_wd_{3}.mod.pkl'.format(base, num, depth, wd)
    ...
    if (encoding is None) and (wd is None) and (pb_encoding is None):
        command_options = f'xreasonBR.py --relax ' + str(relax) + ' -s g3 -z -X abd -R lin -u -N 1 -e mx --drule --fpreprocess -x \'inst\' somefile' 
        command = 'python3 run_axp_mnist_distribute.py -m 1 -n ' + str(num) + ' -d ' + str(depth) + ' -e mx' 
    else:
        # 251209: apply the new used options with wd, pb_encoding, encoding
        command_options = f'xreasonBR.py --relax ' + str(relax) + ' -s g3 -z -X abd -R lin -u -N 1 -e ' + encoding + ' --wdigit ' + str(wd) +\
                        ' --pbencoding ' + str(pb_encoding) + ' --drule --fpreprocess -x \'inst\' somefile'
        command = 'python3 run_axp_mnist_distribute.py -m 1 -n ' + str(num) + ' -d ' + str(depth) + ' -e sat -p ' \
    ...
    if mode == 0:
        ...
        print('log: mxboomer, encode finished!')

        moptions.explain = [float(v.strip()) for v in insts[count].split(',')]
        expl2 = mxboomer.explain(moptions.explain)
        print('log: mxboomer, explanation finished!')

        size_formulas = mxboomer.x.size_formula
        with open(os.path.join(log_folders, base + '_i' + str(count) + '.log'), 'w') as f:
            ...
            f.write(f't: {mxboomer.x.time:.3f}' + '\n')
            f.write(f'c: {mxboomer.x.calls}' + '\n')
            f.write(f'axp: {expl2}' + '\n')
    else:
