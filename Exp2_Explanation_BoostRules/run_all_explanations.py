...
def parse_options():
    ...
    encoding = 'mx' # sat => SAT appraoch, mx => MaxSAT approach
...
if __name__ == '__main__':
    ...
    if not wd is None:
        assert not pb_encoding is None
        ...
        # 260127: just the original version without reducing in weight digits
        results_folder = 'results_AXp_boomer/mx_md' + str(depth) + '_num' + str(num)
        command_options = f'./xreasonBR.py --relax ' + str(relax) + ' -s g3 -z -X abd -R lin -u -N 1 -e ' + encoding + ' --drule --fpreprocess -x \'inst\' somefile'
    ...
    for data in datasets:
        ...
        if not wd is None:
            ...
            mfile = '../Exp1_BoostRules_XGBoost/temp_boomers_251125/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2_wd_{3}.mod.pkl'.format(base, num, adepth, wd)
        else:
            # 251104: fill the path of your Boosted Rules models
            mfile = '../Exp1_BoostRules_XGBoost/temp_boomers_251125/{0}/{0}_maxrule_{1}_maxcond_{2}_testsplit_0.2.mod.pkl'.format(base, num, adepth)
        ...
        # 251006: add the encoding information!!
        # print(f'vars: {mxboomer.mxe.nof_bool_vars}', file=mlog)
        # print(f'clauses: {mxboomer.mxe.nof_clauses}', file=mlog)
        ...
        for i, inst in enumerate(insts):
            # print('The {}-th instance to Explain'.format(i))
        ...
        # 260128: clear the memory when finishing one dataset
        if (not wd is None) and (encoding == 'sat'):
            mxboomer.x.delete()
