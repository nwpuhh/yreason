...
from pysat.formula import CNF, WCNF, IDPool, CNFPlus
...
#==============================================================================
ClassEnc = collections.namedtuple('ClassEnc', ['formula', 'leaves', 'trees', 'ivars'])
...
class MXReasoner:
    ...
    def init(self, encoding, target, solver):
        ...
        for clid in encoding:
            ...
            # LOG: comparing class_i & class_j, 
            # just using the hard clause of class_i, not using all others
            if len(encoding) > 2:
            ...
            # add the size_formula
            clause_orig = len(self.formulas[clid].hard)
...
class SATReasoner:
    ...
    def init_PB_constraints(self, encoding, clid):
        ...
        for label in (clid, self.target):
            ...
            for lit, wght in encoding[label].leaves:        
                # the corresponding leaves weight: w_clid - w_target
                wghts[lit] += coeff * wght
