...
from .explain import SMTExplainer, MXExplainer, SATExplainer
import numpy as np
import os
import resource
...
class XGBooster(object):
    ...
    def __init__(self, options, from_data=None, from_model=None,
        ...
        if not self.options.w_digits is None:
            self.basename += '_wd_' + str(self.options.w_digits)
    ...
    def encode(self, test_on=None):
        ...
        """
        if self.options.encode in ('mx', 'mxe', 'maxsat', 'mxint', 'mxa', 'sat'):
            ...
            if 'x' not in dir(self):
                if self.options.encode in ('mx', 'mxe', 'maxsat', 'mxint', 'mxa'):
                    self.x = MXExplainer(self.enc, self.intvs, self.imaps,
                            ...
                            self.options, self)
                elif self.options.encode == 'sat':
                    self.x = SATExplainer(self.enc, self.intvs, self.imaps, self.ivars, self.feature_names,
                                self.num_class, self.options, self)
                else:
                    self.x = SMTExplainer(self.enc, self.intvs, self.imaps,
                            self.ivars, self.feature_names, self.num_class,
                            self.options, self)
            if not self.options.encode == 'sat':
    ...
    def _postprocess_XGBClassifier_weight_digit(self):
        '''
            260303: the function for limiting the numebr of decimal places allowed in leaf weights
            
            The realization is realized by changing the json file of original model, 
                then the model reload the changed json file
            ATTENTION: the round need for **all float** in the dumped model json, 
                just round the base_weight will fail, as the XGBoost will recaculate the leaf weights again!! 
        '''
        ...
        assert not self.model.get_booster() is None

        def round_floats(obj, wd):
            if isinstance(obj, float):
                return round(obj, wd)
            elif isinstance(obj, dict):
                return {k: round_floats(v, wd) for k, v in obj.items()}
            elif isinstance(obj, list):
                return [round_floats(v, wd) for v in obj]
            else:
                return obj

        model_temp_json = self.basename + '_tree'
        self.model.save_model(model_temp_json + '.json')    
        # change the temp_json
        with open(model_temp_json + '.json', 'r') as f:
            data = json.load(f)
        # just round the base_weights and then reload the model
        data = round_floats(data, wd=self.options.w_digits)
        with open(model_temp_json + 'r.json', 'w') as f:
            json.dump(data, f)

        self.model.load_model(model_temp_json + 'r.json')
        os.remove(model_temp_json + '.json')
        os.remove(model_temp_json + 'r.json')
    ...
    def build_xgbtree(self, outfile=None):
        ...
        # 260304: add the post-processing for the limit the leaf weight decimal places
        if not self.options.w_digits is None:
            self._postprocess_XGBClassifier_weight_digit()
        ...
        # try:
        #     train_accuracy = round(1 - evals_result['validation_0']['merror'][-1], 3)
        # except:
        #     try:
        #         train_accuracy = round(1 - evals_result['validation_0']['error'][-1], 3)
        #     except:
        #         assert(False)

        # try:
        #     test_accuracy = round(1 - evals_result['validation_1']['merror'][-1], 3)
        # except:
        #     try:
        #         test_accuracy = round(1 - evals_result['validation_1']['error'][-1], 3)
        #     except:
        #         print("no results test data")
        #         test_accuracy = 0

        train_accuracy = round(self.model.score(self.transform(self.X_train), self.Y_train), 3)
        test_accuracy = round(self.model.score(self.transform(self.X_test), self.Y_test), 3)
        ...
        return train_accuracy, test_accuracy, self.mod_plainfile
