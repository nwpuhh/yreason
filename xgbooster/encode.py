...
class MXEncoder(SMTEncoder):
    ...
    def traverse(self, tree, clid, prefix=[]):
        ...
        if tree.children:
            var = self.encode_node(tree, clid)
    ...
    def encode_node(self, node, clid):
        """
            Encode a node of a tree.
        """
        ...
        # add the corresponding ivars of clid
        self.enc[clid].ivars.extend([abs(ivar) for ivar in self.ivars[feat]])
    ...
    def encode(self):
        ...
        self.enc = {}
        for j in range(self.nofcl):
            self.enc[j] = ClassEnc(formula=CNF(), leaves=[], trees=[], ivars=[])
