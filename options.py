...
class Options(object):
    ...
    def __init__(self, command):
        ...
        self.exhaust = False
        self.trim = 0
        # 251022: add the option for allowing default rule or not
        self.default_rule = False
        # 251117: add the option of digits cut for weights in Boosted Rules
        self.w_digits = None
        # 251201: add the option of Peudo-Boolean encoding
        self.pb_encoding = 0    # default: 0 -> PBEncType.best
        # 251201: add the option of allowing the formula preprocessing
        self.formula_preprocess = False
    ...
    def parse(self, command):
        ...
        for opt, arg in opts:
            ...
            elif opt == '--fpreprocess':
                self.formula_preprocess = True
            elif opt == '--wdigit':
                self.w_digits = int(arg)
            elif opt == '--pbencoding':
                self.pb_encoding = int(arg)
    ...
    def usage(self):
        ...
        print('        --drule                    Deafault rule will be created without the need of any conditions (default = False)')
        print('        --wdigit                   The digits allowed (post-processed) for weights of Boosted Rules generated. (default: none)')
        print('        --pbencoding               The type of Peudo-Boolean Constraints encoding type applied: 0 -> best, 1 -> bdd, 2 -> seqcounter, ' \
                                                  '3 -> sortnetwrk, 4 -> adder, 5 -> binmerge, 6 -> native')
        print('        --fpreprocess.             Formula preprocessing techniques will be used.')
